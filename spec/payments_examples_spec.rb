...
describe "Payments" do
  ...
  describe "Examples" do
    ...
    describe "Payment_Rest", :integration => true do
      it "Create" do
        payment = Payment_Rest.new(PaymentAttributes)
        # Create
        payment.create
      ...
      it "Create with request_id" do
        ...
        new_payment = Payment_Rest.new(PaymentAttributes.merge( :request_id => request_id ))
        new_payment.create
      ...
      it "Create with token" do
        api = API.new
        payment = Payment_Rest.new(PaymentAttributes.merge( :token => api.token ))
        Payment_Rest.api.should_not eql payment.api
        payment.create
        payment.error.should be_nil
        payment.id.should_not be_nil
      end
      ...
      it "Create with client_id and client_secret" do
        api = API.new
        payment = Payment_Rest.new(PaymentAttributes.merge( :client_id => api.config.client_id, :client_secret => api.config.client_secret))
        Payment_Rest.api.should_not eql payment.api
        payment.create
        payment.error.should be_nil
        payment.id.should_not be_nil
      ...
      it "Find" do
        payment_history = Payment_Rest.all( "count" => 1 )
        payment = Payment_Rest.find(payment_history.payments[0].id)
        payment.error.should be_nil
      end
      ...
      describe "Validation", :integration => true do
        ...
        it "Find with invalid ID" do
          lambda {
            payment = Payment_Rest.find("Invalid")
          }.should raise_error PayPal::SDK::Core::Exceptions::ResourceNotFound
        end

        it "Find with nil" do
          lambda{
            payment = Payment_Rest.find(nil)
          }.should raise_error ArgumentError
        end

        it "Find with empty string" do
          lambda{
            payment = Payment_Rest.find("")
          }.should raise_error ArgumentError
        end

        it "Find record with expired token" do
          lambda {
            Payment_Rest.api.token
            Payment_Rest.api.token.sub!(/^/, "Expired")
            Payment_Rest.all(:count => 1)
    ...
    describe "Future Payment_Rest", :disabled => true do
      access_token = nil
    ...
    describe "Order", :integration => true do
      ...
      before :each, :disabled => true do
        @payment = Payment_Rest.new(PaymentAttributes.merge( "intent" => "order" ))
    ...
    describe "Authorize", :integration => true do
      before :each do
        @payment = Payment_Rest.new(PaymentAttributes.merge( "intent" => "authorize" ))
        @payment.create
        @payment.error.should be_nil
      end
